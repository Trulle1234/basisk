...
class RTError(Error):
    ...
    def generate_traceback(self):
        ...
        while ctx:
            result = f" Fil: {pos.fn}, Linje: {str(pos.ln + 1)}, I: {ctx.display_name}\n" + result
...
class Parser:
    ...
    def if_expr(self):
        res = ParseResult()
        all_cases = res.register(self.if_expr_cases("om"))
        if res.error: return res
        cases, else_case= all_cases
        return res.success(IfNode(cases, else_case))
    
    def if_expr_b(self):
        return self.if_expr_cases("annars_om")
    ...
    def if_expr_b_or_c(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "annars_om"):
    ...
    def while_expr(self):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            if not self.current_tok.matches(TT_KEYWORD, "slut"):
                return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    "Förväntade \"slut\""
                ))
    ...
    def for_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, "för"):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Förväntade \"för\""
            ))
        ...
        if self.current_tok.matches(TT_KEYWORD, "steg"):
            res.register_advancement()
            self.advance()
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, "medan"):
